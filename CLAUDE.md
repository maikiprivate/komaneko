...
- [x] 詰将棋プレイ画面（基本レイアウト）
- [x] 問題情報エリア（ヘッダーに手数・問題番号）
- [x] 進行状況エリア（現在の手数表示）
- [x] 操作ボタンエリア（やり直し・ヒント・解答）
- [x] ナビゲーションエリア（次の問題へ）
- [x] 正解/不正解フィードバックUI（○×表示）

#### Phase 4b: 詰将棋ロジック（作業中）

詳細設計: `docs/designs/tsumeshogi-logic.md`

- [ ] Step 1: 駒の移動ロジック（moveGenerator.ts）
- [ ] Step 2: 王手・詰み判定ロジック（checkmate.ts）
- [ ] Step 3: コンポーネントのタップ対応
- [ ] Step 4: ゲームフック（useTsumeshogiGame.ts）
- [ ] Step 5: 画面統合
- [ ] Step 6: 成りダイアログ
