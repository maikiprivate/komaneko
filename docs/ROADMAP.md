...
### Phase 6: ストリーク更新画面（作業中）

**目標**: 連続学習のゲーミフィケーション

詳細設計: `docs/designs/streak.md`

- [ ] AsyncStorage + ストリーク保存（streakStorage.ts）
- [ ] recordLearningCompletion 共通関数
- [ ] ストリーク更新画面（`/streak-update`）
- [ ] レッスン結果画面への統合
- [ ] 詰将棋画面への統合
- [ ] ホーム画面のストリーク表示更新

### Phase 7: アプリコア（残りのゲーミフィケーション）
- [ ] **core/ 共通モジュール作成**
  - ハートロジック（消費・回復・制限）
  - 学習記録
